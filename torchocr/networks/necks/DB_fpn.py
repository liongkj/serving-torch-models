...
class DB_fpn(nn.Module):
    ...
    def _upsample_add(self, x, y):
        return F.interpolate(x, scale_factor=2.0) + y

    def _upsample_cat(self, p2, p3, p4, p5):
        p3 = F.interpolate(p3, scale_factor=2.0)
        p4 = F.interpolate(p4, scale_factor=4.0)
        p5 = F.interpolate(p5, scale_factor=8.0)
        return torch.cat([p5, p4, p3, p2], dim=1)

    def forward(self, x:List[torch.Tensor]):
        # : Tuple[torch.Tensor,torch.Tensor,torch.Tensor,torch.Tensor]
        # x = torch.jit.annotate(List[torch.Tensor, []])
        c2, c3, c4, c5 = x
